...
def annotate(
    ...
    if isinstance(options, list):
        ...
        if task_type == 'classification':
            ## Added MAISO April 20th, 2022
            if not re_annotating: 
                annotations[value_column] = ''
            ## END Added MAISO
#             annotations[value_column] = '' # COMMENTED BY MAISO April 20th, 2022
        else:
    ...
    def set_label_text(index):
        ...
        labeled = len([label for label in annotations['label'] if not label is None])
        ...
        ## MODIFIED BY MAISO (Feb 1st, 2022) (++ADDED)
        if index==-1:
            str_output =  f'{labeled} of {len(annotations)} Examples annotated.'
            count_label.value=str_output
        ## END MODIFIED BY MAISO
    ...
    def render(index):
        ...
        if index >= len(annotations):
            if stop_at_last_example:
                ...
                for button in buttons:             # ADDED MAISO April 26 2022
                    button.disabled = True         # ADDED MAISO April 26 2022
                    
                ## END MODIFIED BY MAISO
                print('Annotation done.')
                if final_process_fn is not None:
                    # final_process_fn(annotations) COMMENTED by Maiso April 22nd 2022
                    final_process_fn() #            ADDED by Maiso April 22nd 2022
#                 for button in buttons:             # REMOVED MAISO April 26 2022
#                     button.disabled = True         # REMOVED MAISO April 26 2022
                    ## MODIFIED BY MAISO (Feb 1st, 2022) (++ADDED)
    ...
    def add_annotation(annotation):
        ...
        next_example()
    def cancel_labeling(button=None):
        if cancel_process_fn is not None:
            cancel_process_fn()
    ...
    # FUNCTION ADDED BY MAISO Feb 8th, 2022
    def finish(button=None, cancelling=False):
        with out:
            clear_output(wait=True)
            display_fn()
        if  cancelling:
    ...
    # ADDED BY MAISO DATE: June 1st, 2022
    if include_cancel:
        btn = Button(description='cancel', button_style='info')
        btn.on_click(cancel_labeling)
        buttons.append(btn)
    ...
    btn.disabled=True
    buttons.append(btn)
    # END MODIFIED BY MAISO DATE: February 8th, 2022
